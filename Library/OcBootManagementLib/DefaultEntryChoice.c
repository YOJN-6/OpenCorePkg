  ...
  )
{
  ...
  if (!CurrOptionValid) {
    ...
    //
    // Boot0000 seems to be reserved on some platforms (e.g. Apple uses it in
    // BootCurrent to signal "default" and it not used on some laptop InsydeH2O
    // firmwares). If it is supported, it is likely already occupied anyway.
    //
